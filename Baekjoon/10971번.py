...
def dfs(start, next, value, cnt, visited):
    global min_value
    ...
    for i in range(N):
        if travel_cost[next][i] != 0 and visited[i] == False:
            if value < min_value:
                visited[i] = True
                dfs(start, i, value+travel_cost[next][i], cnt+1, visited)
                visited[i] = False
